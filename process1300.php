...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   $sqlExportToday = "SELECT * FROM datos WHERE DATE(fecha_creacion) = CURDATE() AND centro LIKE '1300' AND tipo != '2' AND fecha_creacion BETWEEN '$startDateTime' AND '$endDateTime'";
   $resultadoExportToday = $conexion->query($sqlExportToday);

   if ($resultadoExportToday && $resultadoExportToday->num_rows > 0) {
       $filename = "inv_1300_" . $fechaArchivo . "_p1.csv"; // Nombre del archivo con la fecha determinada
