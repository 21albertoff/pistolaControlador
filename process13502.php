...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($valores as $v) {
        ...
        if (!empty($v)) {
            ...
            $sqlVerificar = "SELECT COUNT(*) as count FROM datos WHERE valor = '$v' AND centro LIKE '1350' AND tipo = '2' AND DATE(fecha_creacion) = CURDATE()";
   ...
   $sqlExportToday = "SELECT * FROM datos WHERE DATE(fecha_creacion) = CURDATE() AND centro LIKE '1350' AND tipo = '2' AND fecha_creacion BETWEEN '$startDateTime' AND '$endDateTime'";
   $resultadoExportToday = $conexion->query($sqlExportToday);

   if ($resultadoExportToday && $resultadoExportToday->num_rows > 0) {
       $filename = "inv_1350_" . $fechaArchivo . "_p2.csv"; // Nombre del archivo con la fecha actual
