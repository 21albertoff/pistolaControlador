...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($valores as $v) {
        ...
        if (!empty($v)) {
            ...
            $sqlVerificar = "SELECT COUNT(*) as count FROM datos WHERE valor = '$v' AND centro LIKE '1500' AND DATE(fecha_creacion) = CURDATE()";
    ...
    $sqlExport = "SELECT * FROM datos WHERE centro LIKE '1500' AND fecha_creacion BETWEEN '$startDateTime' AND '$endDateTime'";
    $resultadoExport = $conexion->query($sqlExport);

    if ($resultadoExport && $resultadoExport->num_rows > 0) {
        $filename = "inv_1500_" . $fechaArchivo . ".csv"; // Nombre del archivo con la fecha determinada
